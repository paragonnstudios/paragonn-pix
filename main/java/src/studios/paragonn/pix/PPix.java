...
import studios.paragonn.pix.commands.APMenuCommand;
import studios.paragonn.pix.commands.PPixCommand;
import studios.paragonn.pix.commands.ProductCommand;
import studios.paragonn.pix.domain.Order;
import studios.paragonn.pix.domain.PixData;
import studios.paragonn.pix.expansion.PPixExpansion;
...
public class PPix extends JavaPlugin {
	...
	public void onEnable() {
		...
		getCommand("autopix").setExecutor(new PPixCommand());
		getCommand("autopixmenu").setExecutor(new APMenuCommand());
		getCommand("produto").setExecutor(new ProductCommand());
		
		Bukkit.getPluginManager().registerEvents(new InventoryListener(), this);
		
		if (Bukkit.getPluginManager().isPluginEnabled("PlaceholderAPI")) {
			new PPixExpansion().register();
		}
	}
	
	public static PPix getInstance() {
		return instance;
	}
	...
	public static boolean reloadPlugin() {
		...
		if (Bukkit.getPluginManager().isPluginEnabled("PlaceholderAPI")) {
			...
			HOLOGRAM_TASK = new BukkitRunnable() {		
				@Override
				public void run() {
					PPixExpansion.updateTopDonorsCache(OrderManager.getTopDonors());
				}
